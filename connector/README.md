# WebRTCConnector Unity Plugin

This repository contains the Unity plugin that can be used to communicate with the WebRTC reference client when using Unity. You can build the plugin with Visual Studio (make sure you build the release version and not the debug one) and copy the .dll file to your Unity application.
...
void Awake()
{
    ...
    nativeLibraryPtr = Native.LoadLibrary("WebRTCConnector");
